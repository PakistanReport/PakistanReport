...
`npm test` covers ingestion through validated export, realistic negative fixtures, source restrictions, model-response parsing, private HTTP access, state transitions, retries and DOM review controls. `npm run test:publisher` runs the unchanged Publisher regression suite. `npm run check` bundles this separate Worker without deploying. `npm run test:live` reports disabled entries and makes one-shot attempts only for explicitly enabled, validated source configurations. It never supplies a restrictions review or enables a source itself. Results go under ignored `test-output/`.
...
| Manual evidence | Editor checks primary documents or supplies short structured reporting facts | Supreme Court, PSX disclosures, Reuters corroboration; PDF documents |
...
Reporting sources provide headlines/links only through automatic ingestion. Their full articles are not crawled or used as rewrite inputs. Primary evidence is preferred. Reviewed source ownership groups, rather than URL counts or unreviewed owner labels, determine independent corroboration. Reporting evidence requires editor-assembled `statement`, `excerpt`, comparison `key` and `value`, plus a provenance note and attestation. The service limits reporting excerpts to 25 words each and 100 words per source, rejects article-only input, and does not accept legacy reporting documents as evidence. These collection limits do not grant rights to use a source.

Clustering is heuristic (shared terms and a bounded event-time window), not an oracle. Related reports retain separate evidence and conflicting values. Use **Split into separate candidate** if unrelated developments were grouped. Identical observations are ignored; changed source content creates a new revision and invalidates approval; timestamp-only refreshes retain the original age and revision. Same-URL updates join the existing candidate. Document changes that do not change the feed/index metadata are not independently detected; editors must refresh evidence for corrections.
...
## Discovery / evidence separation and access audit

The [26-source access audit](RADAR-AUDIT.md) supersedes earlier assumptions that a working RSS endpoint is sufficient permission. No new mainstream feed is enabled. Automated reporting connectors now require a documented `discoveryPermission` review as well as robots and interval checks. Official sources default to `purpose: evidence`; automatic ticks select `purpose: radar` only. Existing reviewed official sources remain available for explicit research polling.

The model receives independently assembled evidence records, not a publisher article to rewrite. Primary document excerpts remain unverified until claim checks and human review; no general automatic web-research engine is claimed. Explicit conflicting fact records cannot be silently omitted from a draft. Original-headline and retained-wording checks flag possible copying for revision; they cannot certify originality or legal compliance. See the audit for exact source outcomes, configuration, remaining World/Pakistan coverage gaps and the next supervised test.


## Desired mainstream radar

The [discovery expansion](DISCOVERY-EXPANSION.md) adds machine-readable coverage gaps for all requested priority outlets, World sources and supporting references. **33 entries, zero enabled by default.** The private registry displays tiers, coverage, ownership and permission/blocking state. Existing endpoints and local source settings are preserved. Reporting permission must now name the exact endpoint and allowed hosts; unknown ownership does not receive independence credit. Read the expansion's supervised-test procedure before using the local model: it stops at human review, and a missing reviewed visual remains blocking.
